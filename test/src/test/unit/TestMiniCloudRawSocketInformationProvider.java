 ...
 */
public class TestMiniCloudRawSocketInformationProvider
{
	private static final transient Logger Log = Logger.getLogger( TestMiniCloudRawSocketInformationProvider.class.getName() );
				...
				new Runnable()
				{
					public void run()
					{
						try
						{
							...
							TestMiniCloudRawSocketInformationProvider.this.isServer.start();
						}
						catch (Exception e)
						{
	...
	public void testIsRunning() throws Exception
	{
		...
		RawSocketInformationProvider isClient = new RawSocketInformationProvider( this.isServer.getPort() );

		try
		{
			Log.info("[IS-Client>> Requesting if server is running...");

			if ( isClient.isRunning() )
			{
				Log.info("[IS-Client>> Server running.");
			}
			else
			{
				Log.info("[IS-Client>> Server not running.");
			}

			Log.info("[IS-Client>> Requested if server is running.");

			allOk = true;
		}
		catch (Exception e)
		{
			Log.log(Level.WARNING, "[IS-Client>> Caught exception: ", e);
		}
	...
	public void testServerProtocolVersionRequest()
	{
		...
		try
		{
			...
			{
				Log.info("[IS-Client>> Server seems not to be running (trial #" + (i+1) + "). Sleeping...");
				Thread.sleep(5);
			}

			if ( this.isServer.isRunning() )
			{
				Log.info("[IS-Client>> Requesting server protocol version...");
				...
				Log.info("[IS-Client>> Requested server protocol version.");
			}
			else
			{
				Log.info("[IS-Client>> Server not running...");
	...
	public void testListServices()
	{
		...
		try
		{
			...
			if ( this.isServer.isRunning() )
			{
				Log.info("[IS-Client>> Requesting all services...");
				...
				Log.info("[IS-Client>> Requested all services.");
			}
			else
			{
				Log.info("[IS-Client>> Server not running...");
	...
	public void testListPhysicalMachines()
	{
		...
		try
		{
			...
			if ( this.isServer.isRunning() )
			{
				Log.info("[IS-Client>> Requesting all physical machines...");
				...
				Log.info("[IS-Client>> Requested all physical machines.");
			}
			else
			{
				Log.info("[IS-Client>> Server not running...");
	...
	{
		org.junit.runner.JUnitCore.main( TestMiniCloudRawSocketInformationProvider.class.getName() );
	}
}
